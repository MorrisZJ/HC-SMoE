...
def _load_c4():
    # Official instruction (https://github.com/wazenmai/HC-SMoE): download C4 from allenai/c4,
    # save to hcsmoe/data/c4-train.00000-of-01024.json. If that file exists, use it; else load from HF.
    _data_dir = os.path.join(os.path.dirname(__file__), "..", "data")
    _local_c4 = os.path.abspath(os.path.join(_data_dir, "c4-train.00000-of-01024.json"))
    if os.path.isfile(_local_c4):
        return load_dataset("json", data_files={"train": _local_c4}, trust_remote_code=True)
    logger.warning(
        f"Local C4 not found at {_local_c4}. Loading from HuggingFace (allenai/c4); "
        "this may download large shards. To use local data, download from allenai/c4 and save to the path above."
    )
    ds = load_dataset("allenai/c4", "en", split="train[:2048]", trust_remote_code=True)
