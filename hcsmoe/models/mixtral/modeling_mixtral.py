...
def load_balancing_loss_func(
    ...
    if isinstance(gate_logits, tuple):
        ...
        print("gate_logits: ", len(gate_logits))
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config: MixtralConfig, layer_index):
        ...
        _gne = getattr(config, "gate_num_experts", None)
        if _gne is not None:
            self.gate_num_experts = _gne[layer_index] if isinstance(_gne, list) else _gne
...
class MixtralDecoderLayer(nn.Module):
    def __init__(self, config: MixtralConfig, layer_idx: int):
        ...
        # 🔍
        _lex = getattr(config, "layer_experts_idx", None)
        if _lex is not None:
            num_experts = -1 if _lex[layer_idx] is None else len(_lex[layer_idx])
        else:
...
class MixtralModel(MixtralPreTrainedModel):
    ...
    def forward(
        ...
        # print(f"self.embed_tokens: {self.embed_tokens.weight.size()}")
...
class MyMixtralForCausalLM(MixtralPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # num_local_experts may be a per-layer list after pruning; store the max for loss computation
        _nle = config.num_local_experts
        self.num_experts = max(_nle) if isinstance(_nle, list) else _nle
...
# Public alias used for AutoModel registration and save_pretrained / from_pretrained.
MixtralForCausalLM = MyMixtralForCausalLM